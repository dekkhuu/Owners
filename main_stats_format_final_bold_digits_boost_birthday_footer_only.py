...
async def birthday(
    ...
    embed = discord.Embed(
        description="**Các mem hãy bấm nút đăng ký để nhập ngày tháng năm sinh của mình**",
        color=discord.Color.blurple()
    )
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("❌ Thiếu tham số. Hãy kiểm tra cú pháp lệnh.", delete_after=5)
        return
    if isinstance(error, commands.BadArgument):
        await ctx.send("❌ Tham số không hợp lệ. Hãy kiểm tra lại.", delete_after=5)
        return
